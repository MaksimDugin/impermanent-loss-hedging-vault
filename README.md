...
В `test/Backtest.t.sol` сейчас зашиты 3 сценария: downtrend→recovery, high-vol chop и up-only trend.
По последнему запуску (логи из `forge test --match-contract BacktestVaultScenarios -vv`) картина такая:

- **DowntrendThenRecovery**
  - `HOLD`: `4050.0000` USD
  - `LP`: `4049.6913` USD (`lpTrackingErrorBps = 0`)
  - `HEDGE`: `4049.1859` USD (`hedgedTrackingErrorBps = 2`)
- **HighVolatilityChop**
  - `HOLD`: `4000.0000` USD
  - `LP`: `3999.9999966` USD (`lpTrackingErrorBps = 0`)
  - `HEDGE`: `3999.6712` USD (`hedgedTrackingErrorBps = 0` после integer-округления bps)
- **UpOnlyTrend**
  - `HOLD`: `5200.0000` USD
  - `LP`: `5059.6443` USD (`lpTrackingErrorBps = 269`)
  - `HEDGE`: `5058.3621` USD (`hedgedTrackingErrorBps = 272`)

**Вывод по текущему MVP:**
- В боковике LP почти совпадает с hold, а hedge даёт небольшой дополнительный минус.
- В ап-тренде обе LP-стратегии проигрывают hold из-за IL, и текущий hedge чуть хуже plain LP.
- Это согласуется с тем, что в тесте хедж не моделирует полные рыночные издержки и при этом не всегда точно улучшает PnL.

Иными словами, на текущем наборе параметров `rebalance()` больше выполняет функцию **контроля дельты/риска**, чем функцию гарантированного улучшения доходности относительно plain LP.

Сравнение строится по трём кривым:
- `hold value = initial ETH * current price + initial USDC`
- `LP value = value of the LP share`
- `hedged value = LP value + hedge PnL - fees`

Остаточный эффект (и расхождение с hold) по-прежнему объясняется кривизной LP-позиции:

$$
\frac{1}{2}\Gamma\sigma^2
$$

что неизбежно для constant product AMM даже при периодическом хедже.
